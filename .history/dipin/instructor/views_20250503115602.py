...
class QuizDetailView(View):
    def get(self, request, class_shell_id, quiz_id):
        ...
        # Preprocess MCQ answers for the template
        for question in questions:
            if question.type == 'multiple_choice' and question.mcq_answers:
                question.mcq_answers_display = ', '.join(question.mcq_answers)
    ...
    def post(self, request, class_shell_id, quiz_id):
        ...
        if 'add_question' in request.POST and question_form.is_valid():
            ...
            if question_instance.type == 'multiple_choice':
                print('ayo')
                # Process choices from textarea (split by newlines)
                choices_text = question_form.cleaned_data['choices']
                choices = [choice.strip() for choice in choices_text.split('\n') if choice.strip()]
                question_instance.mcq_choices = choices
                
                # Process selected answers
                mcq_answers = question_form.cleaned_data['mcq_answers']
                question_instance.mcq_answers = mcq_answers
                
                # Set answer field to first correct answer (or empty if none selected)
                question_instance.answer = mcq_answers[0] if mcq_answers else ''
                
            elif question_instance.type == 'true_false':
                question_instance.tf_answer = question_form.cleaned_data['tf_answer'] == 'True'
                question_instance.answer = str(question_instance.tf_answer)
                question_instance.mcq_choices = None
                question_instance.mcq_answers = None
                
            elif question_instance.type == 'essay':
                question_instance.answer = question_form.cleaned_data['essay_answer']
                question_instance.mcq_choices = None
                question_instance.mcq_answers = None
            ...
            question_instance.mark = question_form.cleaned_data['mark']
