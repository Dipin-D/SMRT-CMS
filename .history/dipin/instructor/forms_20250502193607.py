...
<div class="container mt-4">
  ...
  <form method="POST" id="question-form">
    ...
    <div id="multiple_choice_fields" style="display: {% if question_form.type.value == 'multiple_choice' %}block{% else %}none{% endif %};">
      ...
      <div class="mb-3">
        {{ question_form.mcq_answer.label }}
        <div class="mb-3">
          <label for="id_mcq_answer">Correct Answers (You can select multiple):</label>
          {{ question_form.mcq_answer }}
        </div>
      </div>        
    </div>
        ...
        <div>
          ...
          <button class="btn btn-warning shadow-sm edit-button" 
                  ...
                  {% if question.type == 'multiple_choice' %} 
                    ...
                    data-correct="{{ question.mcq_answer|join:', ' }}"
...
<script>
  ...
  document.querySelectorAll('.edit-button').forEach(button => {
    button.addEventListener('click', function() {
        ...
        if (questionType === 'multiple_choice') {
            ...
            document.getElementById('id_mcq_answer').value = this.getAttribute('data-correct') || "";
  ...
  // Call updateFields on page load to set the initial state
