        ...
        if 'add_question' in request.POST and question_form.is_valid():
            ...
            if question_instance.type == 'multiple_choice':
                if question_instance.type == 'multiple_choice':
                    ...
                    mcq_answers_list = question_form.cleaned_data['mcq_answers']
                    # Strip whitespace from each answer
                    mcq_answers = [answer.strip() for answer in mcq_answers_list]
                    print('mcqanswer', mcq_answers)
                    # Check if selected answers match the choices and store the corresponding choices
                    selected_answers = []
                    if 'Choice 1' in mcq_answers:
                        selected_answers.append(question_instance.choice_1)
                    if 'Choice 2' in mcq_answers:
                        selected_answers.append(question_instance.choice_2)
                    if 'Choice 3' in mcq_answers:
                        selected_answers.append(question_instance.choice_3)
                    if 'Choice 4' in mcq_answers:
                        selected_answers.append(question_instance.choice_4)
                    # Directly save the list of selected answers into the JSONField
                    question_instance.mcq_answers = selected_answers
            ...
            if question_instance.type == 'multiple_choice':
                ...
                question_instance.choice_3 = question_form.cleaned_data['choice_3']
                question_instance.choice_4 = question_form.cleaned_data['choice_4']
                existing_answers = question_instance.mcq_answers or []
                # This will pre-populate the choices in the form based on the existing answers
                question_form.fields['mcq_answers'].initial = existing_answers
                # Now save the updated mcq_answers field
                mcq_answers_list = question_form.cleaned_data['mcq_answers']
                mcq_answers = [answer.strip() for answer in mcq_answers_list]  # Clean up spaces
                selected_answers = []
                # Match the answers with choices and store them in the list
                if 'Choice 1' in mcq_answers:
                    selected_answers.append(question_instance.choice_1)
                if 'Choice 2' in mcq_answers:
                    selected_answers.append(question_instance.choice_2)
                ...
                if 'Choice 4' in mcq_answers:
                    selected_answers.append(question_instance.choice_4)
                # Save the selected answers to the JSONField
                question_instance.mcq_answers = selected_answers
                print('selctedanswers',selected_answers)
            elif question_instance.type == 'true_false':
                question_instance.mcq_answer = None  
