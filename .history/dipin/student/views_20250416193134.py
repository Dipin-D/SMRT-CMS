...
def while_quiz(request, class_shell_id, quiz_id):
    ...
    # ✅ If time expired and not yet submitted, auto-submit
    if remaining_time <= 0 and not quiz_attempt.submitted:
        score = 0
        for question in questions:
            # Use previously saved answers via AJAX
            try:
                q_attempt = QuestionAttempt.objects.get(quiz_attempt=quiz_attempt, question=question)
                user_answer = q_attempt.student_answer
                is_correct = False
                ...
                # Essay can be left as is (subjective)

                q_attempt.is_correct = is_correct
                q_attempt.save()
    ...
    return render(request, 'while_quiz.html', {
        ...
        'current_attempt': quiz_attempt.attempt_number,
    })
...
def save_answer_ajax(request):
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        try:
            ...
            quiz_attempt = QuizAttempt.objects.filter(student=request.user, quiz=quiz, submitted=False).latest('attempt_number')

            question_attempt, _ = QuestionAttempt.objects.update_or_create(
                quiz_attempt=quiz_attempt,
                question=question,
                defaults={
