...
class go_to_course_student(View):
    def get(self, request, class_shell_id):
        ...
        # Donut chart: What percentage of marks student earned vs left behind
        labels = ['Earned Marks', 'Missed Marks']

        earned = submitted_assignment_score + submitted_quiz_score
        possible = submitted_assignment_marks + submitted_quiz_marks
        missed = possible - earned
        ...
        if possible == 0:
            ...
            values = [earned, missed]

        fig = go.Figure(data=[go.Pie(
            labels=labels,
            values=values,
            hole=0.4,
            marker=dict(colors=['#008000', '#FF6347']),  # green = earned, red = missed
            hoverinfo="label+percent",
            textinfo="label+value"
        )])

        fig.update_layout(
            title="Your Progress Toward Full Marks",
            title_x=0.5,
