...
def while_quiz(request, class_shell_id, quiz_id):
    ...
    print('quiz attempt id', quiz_attempt.id)
    ...
    print('saved answer', saved_answers)
...
def autosave_answer(request):
    print("Received data:", request.POST)  # Debug what's actually received
    if request.method != 'POST':
    ...
    if request.method == 'POST':
        ...
        if not question_id or not selected_answer or not quiz_attempt_id:
        ...
        try:
            ...
            question_attempt, created = QuestionAttempt.objects.get_or_create(
                quiz_attempt=quiz_attempt,
                question=question,
                defaults={'student_answer': selected_answer}
            )
        ...
        except (QuizAttempt.DoesNotExist, Question.DoesNotExist) as e:
