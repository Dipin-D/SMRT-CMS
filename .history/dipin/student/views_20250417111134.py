...
def while_quiz(request, class_shell_id, quiz_id):
    ...
    # Handle expired quiz that wasn't submitted
    if current_time >= quiz_attempt.end_time and not quiz_attempt.submitted:
        all_questions = quiz.questions.all()
        answered_ids = QuestionAttempt.objects.filter(quiz_attempt=quiz_attempt).values_list('question_id', flat=True)

        score = 0
        for question in all_questions:
            if question.id in answered_ids:
                attempt = QuestionAttempt.objects.get(quiz_attempt=quiz_attempt, question=question)
    ...
    # Pre-fill questions with saved answers
