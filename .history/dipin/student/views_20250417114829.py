...
def while_quiz(request, class_shell_id, quiz_id):
    ...
    # Check if time has expired, and if so, automatically submit
    if remaining_time <= 0:
        return handle_time_expired(request, class_shell, quiz, quiz_attempt)
...
def handle_time_expired(request, class_shell, quiz, quiz_attempt):
    questions = Question.objects.filter(quiz=quiz)
    score = 0
    for question in questions:
        # Get saved answers for the expired quiz attempt
        try:
            user_answer = QuestionAttempt.objects.get(quiz_attempt=quiz_attempt, question=question).student_answer
        except QuestionAttempt.DoesNotExist:
            user_answer = 'n/a'  # Default answer if not selected

        # If no answer was selected, mark it as 'n/a'
        if user_answer == '' or user_answer is None:
            user_answer = 'n/a'

        # Check if the answer is correct (only if it's not 'n/a')
        is_correct = False
        if user_answer != 'n/a':
            if question.type == "multiple_choice":
                is_correct = (user_answer == question.mcq_answer)
        ...
        # Update or create the question attempt with the student's answer and correctness
        QuestionAttempt.objects.update_or_create(
            quiz_attempt=quiz_attempt,
            question=question,
            defaults={
    ...
    # Auto-submit quiz attempt after time expires
    quiz_attempt.score = score
    quiz_attempt.submitted = True
    quiz_attempt.save()

    return redirect('student:attempt_quiz', class_shell_id=class_shell.id, quiz_id=quiz.id)
