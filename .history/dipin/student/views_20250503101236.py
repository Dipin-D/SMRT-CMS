    ...
    if request.method == 'POST':
        ...
        for question in questions:
            if question.type == "multiple_choice":
                user_answers = []
                for i, choice in enumerate([question.choice_1, question.choice_2, question.choice_3, question.choice_4], start=1):
                        if f'answer_{question.id}' in request.POST and str(choice) in request.POST.getlist(f'answer_{question.id}'):
                            user_answers.append(f"choice_{i}")
                user_answer_str = ','.join(user_answers) if user_answers else 'n/a'
                
                # Compare with correct answers (assuming mcq_answers is a comma-separated string)
                correct_answers = set(question.mcq_answers)  
                user_answers_set = set(user_answers)
                print('select',user_answers_set,'actualanswer',question.mcq_answers)
