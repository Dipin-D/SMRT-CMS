...
def while_quiz(request, class_shell_id, quiz_id):
    # Retrieve the class shell and quiz
    class_shell = get_object_or_404(ClassShell, id=class_shell_id)
    quiz = get_object_or_404(Quiz, id=quiz_id, class_shell=class_shell)
    questions = Question.objects.filter(quiz=quiz)

    # Get latest attempt for the quiz
    ...
    # Pass saved answers to the template to pre-fill the form
    for question in questions:
        question.saved_answer = saved_answers.get(question.id)

    # If the time expired and the form is not submitted yet, auto-submit it
    if remaining_time <= 0 and not quiz_attempt.submitted:
        # Auto-submit quiz if time is up
        score = 0
        for question in questions:
            user_answer = saved_answers.get(question.id, 'n/a')
            is_correct = False
            if user_answer != 'n/a':
    ...
    # Handle form submission for answers (when the user submits the quiz manually)
