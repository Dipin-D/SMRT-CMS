...
class go_to_course_student(View):
    def get(self, request, class_shell_id):
        ...
        # Gather real grades from latest assignment and quiz submissions (as shown in the table)
        assignment_chart_grades = [
            sub['grade'] for sub in latest_assignments_info if sub['grade'] is not None
        ]
        assignment_chart_total = [
            sub['total_marks'] for sub in latest_assignments_info if sub['grade'] is not None
        ]

        quiz_chart_grades = [
            sub['grade'] for sub in submitted_quizzes_info if sub['grade'] is not None
        ]
        quiz_chart_total = [
            sub['total_marks'] for sub in submitted_quizzes_info if sub['grade'] is not None
        ]

        # Calculate totals based on actual grades
        assignment_earned = sum(assignment_chart_grades)
        assignment_possible = sum(assignment_chart_total)

        quiz_earned = sum(quiz_chart_grades)
        quiz_possible = sum(quiz_chart_total)
        ...
        values_a = [assignment_earned, assignment_possible - assignment_earned] if assignment_possible else [1]
        ...
        values_q = [quiz_earned, quiz_possible - quiz_earned] if quiz_possible else [1]
        ...
        # Convert to HTML
        interactive_overall_chart = opy.plot(fig_a, output_type='div', include_plotlyjs=False)
...
def while_quiz(request, class_shell_id, quiz_id):
    ...
    saved_answers = {
        qa.question.id: qa.student_answer.split(',') if qa.student_answer else []
        for qa in QuestionAttempt.objects.filter(quiz_attempt=quiz_attempt)
    }
    print('mathi ko suru ko saved answers',saved_answers)
    ...
    for question in questions:
        ...
        # Ensure question attempt exists
    ...
    if request.method == 'POST':
        ...
        for question in questions:
            if question.type == "multiple_choice":
                ...
                # Store exactly what was selected
                print('selected choices',selected_choices)
                user_answer_str = ','.join(selected_choices) if selected_choices else 'n/a'
                # Compare with correct answers (now using the same "Choice X" format)
                correct_answers = set(question.mcq_answers) if question.mcq_answers else set()
                is_correct = set(selected_choices) == correct_answers
            else:
                user_answer_str = request.POST.get(f'answer_{question.id}', 'n/a')
                if question.type == "true_false":
                    is_correct = (str(user_answer_str) == str(question.tf_answer))
                else:  # essay
                    is_correct = False  # Essays typically need manual grading
...
def autosave_answer(request):
    ...
    try:
        ...
        if question.type == "multiple_choice":
            print('select',selected_answer,'actualanswer',question.mcq_answers)
            is_correct = (selected_answer == question.mcq_answers)
        elif question.type == "true_false":
            print('select',selected_answer,'actualanswer',question.tf_answer)
...
def while_exercise(request, class_shell_id, exercise_id):
    ...
    saved_answers = {
        eaq.exercise_question.id: eaq.student_answer.split(',') if eaq.student_answer else []
        for eaq in ExerciseQuestionAttempt.objects.filter(exercise_attempt=exercise_attempt)
    }
    print('saved_answers',saved_answers)

    for question in questions:
        question.saved_answer_exercise = saved_answers.get(question.id, "")
    ...
    if request.method == 'POST':
        ...
        for question in questions:
            ...
            if question.type == "multiple_choice":
                selected_choices = request.POST.getlist(f'answer_{question.id}')
                user_answer = ','.join(selected_choices) if selected_choices else 'n/a'
                correct_answers = set(question.mcq_answers) if question.mcq_answers else set()
                is_correct = set(selected_choices) == correct_answers
            else:
...
def autosave_answer_exercise(request):
    ...
    try:
        ...
        if exercise_question.type == "multiple_choice":
            # Always treat selected_answer as a comma-separated string of answers
            selected_list = selected_answer.split(',') if selected_answer else []
            correct_list = exercise_question.mcq_answers if exercise_question.mcq_answers else []

            print("🧪 AUTOSAVE DEBUG")
            print("Selected:", selected_list)
            print("Correct:", correct_list)

            is_correct = set(selected_list) == set(correct_list)

            saved_string = ','.join(selected_list)

        elif exercise_question.type == "true_false":
            print("🧪 AUTOSAVE DEBUG - TRUE/FALSE")
            print("Selected:", selected_answer)
            print("Correct:", exercise_question.tf_answer)

            is_correct = (selected_answer == str(exercise_question.tf_answer))
            saved_string = selected_answer

        elif exercise_question.type == "essay":
            saved_string = selected_answer  # just store the full response

        else:
            saved_string = selected_answer  # default fallback

        ExerciseQuestionAttempt.objects.update_or_create(
            exercise_attempt=exercise_attempt,
            exercise_question=exercise_question,
            defaults={
                'student_answer': saved_string,
                'is_correct': is_correct
            }
        )
